...
nuCur = 0.001*255*255; % the weight of the length term
...
phi = phi0;
Bhatt = ones(iterNum, 1);
savedPhi = zeros(2, h, w);
pause();

for k = 1:iterNum
    [phi, pOut, pIn] = Bhattacharyya(Img, phi, sigma, nuCur, nuP, epsilon, timeStep);
    ...
    % save the Bhattacharyya energy
    Bhatt(k) = sum ( sqrt(pOut .* pIn) );
    
    % determine whether to stop iteration
    if k >= 5
        if Bhatt(k - 1) <= Bhatt(k) || any( any( isnan(phi)) )
...
% plot Bhattacharyya Energy
figure(3);
plot(Bhatt(1:k-1));
suptitle('Bhattacharyya Energy');